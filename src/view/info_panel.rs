...
mod imp {
    ...
    impl ObjectSubclass for InfoPanel {
        ...
        fn class_init(klass: &mut Self::Class) {
            ...
            klass.install_action_async(ACTION_REFRESH, None, async |widget, _, _| {
    ...
    impl InfoPanel {
        #[template_callback]
        async fn on_notify_client(&self) {
            self.obj().refresh().await;
        }
    }
}
    ...
    pub(crate) async fn refresh(&self) {
        ...
        .await
        {
            Ok(info) => {
                ...
                imp.version_api_version_row
                    .set_value(&utils::format_option(info.api_version.as_ref()));
                ...
                imp.version_git_commit_row.set_value(&utils::format_option(
                    info.git_commit.as_ref().and_then(
                        |s| {
                            if s.is_empty() { None } else { Some(s) }
                        },
                    ),
                ));
                imp.version_go_version_row
                    .set_value(&utils::format_option(info.go_version.as_ref()));
                imp.version_os_arch_row
                    .set_value(&utils::format_option(info.os_arch.as_ref()));
                imp.version_version_row
                    .set_value(&utils::format_option(info.version.as_ref()));
                ...
                //     .set_label(&utils::format_option(container_store.and_then(|c| {
                //         c.number.map(|n| {
                ...
                //         container_store.and_then(|s| s.paused.as_ref().map(i64::to_string)),
                //     ));
                // imp.store_container_store_running_row
                //     .set_value(&utils::format_option(
                //         container_store.and_then(|s| s.running.as_ref().map(i64::to_string)),
                //     ));
                // imp.store_container_store_stopped_row
                //     .set_value(&utils::format_option(
                //         container_store.and_then(|s| s.stopped.as_ref().map(i64::to_string)),
                //     ));
                ...
                //     .set_label(&utils::format_option(graph_options.as_ref().map(|o| {
                //         let len = o.as_object().unwrap().len();
                //         // Translators: "{}" is a placeholder for a cardinal number.
                //         ngettext!("{} Option", "{} Options", len as u32, len)
                //     })));

                // let mut store_graph_options_rows = imp.store_graph_options_rows.borrow_mut();
                ...
                //     .set_label(&utils::format_option(store.and_then(|s| {
                //         s.graph_status.as_ref().map(|s| {
                ...
                //     });
                // }
                // imp.store_image_store_row.set_value(&utils::format_option(
                //     store.and_then(|s| s.image_store.as_ref()).and_then(|s| {
                //         s.number.map(|n| {
                //             // Translators: "{}" is placeholders for a cardinal number.
            ...
            Err(e) => {
                ...
                utils::show_error_toast(self, &gettext("Error on retrieving info"), &e.to_string());
            }
        }

        self.action_set_enabled(ACTION_REFRESH, true);
    }
}
