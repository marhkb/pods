                    ...
                    move |result| {
                        ...
                        match result {
                            Ok(info) => {
                                ...
                                imp.version_built_time_row.set_value(&utils::format_option(
                                    version.and_then(|v| {
                                        v.built.and_then(|t| {
                                ...
                                let container_store =
                                    store.and_then(|s| s.container_store.as_ref());
                                ...
                                imp.store_container_store_label
                                    .set_label(&utils::format_option(container_store.and_then(
                                        |c| {
                                            c.number.map(|n| {
                                ...
                                imp.store_container_store_paused_row.set_value(
                                    &utils::format_option(
                                        container_store
                                            .and_then(|s| s.paused.as_ref().map(i64::to_string)),
                                    ),
                                );
                                imp.store_container_store_running_row.set_value(
                                    &utils::format_option(
                                        container_store
                                            .and_then(|s| s.running.as_ref().map(i64::to_string)),
                                    ),
                                );
                                imp.store_container_store_stopped_row.set_value(
                                    &utils::format_option(
                                        container_store
                                            .and_then(|s| s.stopped.as_ref().map(i64::to_string)),
                                    ),
                                );
                                ...
                                imp.store_graph_options_label
                                    .set_label(&utils::format_option(graph_options.as_ref().map(
                                        |o| {
                                ...
                                if let Some(graph_options) =
                                    store.and_then(|s| s.graph_options.as_ref())
                                {
                                    graph_options
                                        .as_object()
                                        .unwrap()
                                ...
                                let mut store_graph_status_rows =
                                    imp.store_graph_status_rows.borrow_mut();
                                while let Some(row) = store_graph_status_rows.pop() {
                                    imp.store_graph_status_row.remove(&row);
                                }

                                if let Some(graph_status) =
                                    store.and_then(|s| s.graph_status.as_ref())
                                {
