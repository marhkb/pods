    ...
    impl ObjectImpl for ActionRow {
        ...
        fn constructed(&self) {
            ...
            let action_description_expr = action_expr.chain_closure::<String>(
                closure!(|_: Self::Type, action: Option<&model::Action>| {
                    action
                    ...
                    |_: Self::Type, state: model::ActionState| {
                        classes
                            ...
                            .chain(Some(String::from(match state {
                                model::ActionState::Cancelled => "warning",
                                model::ActionState::Failed => "error",
                                model::ActionState::Finished => "dim-label",
                                model::ActionState::Ongoing => "accent",
                            })))
                            .collect::<Vec<_>>()
                    }
                ))
            ...
            action_state_expr
                .chain_closure::<String>(closure!(|_: Self::Type, state: model::ActionState| {
                    if state == model::ActionState::Cancelled {
...
fn action_image(action: &model::Action) -> &str {
    if action
        .downcast_ref::<model::ContainerCommitAction>()
        .is_some()
    {
        "merge-symbolic"
    } else if action
        .downcast_ref::<model::ContainerCopyFromAction>()
        .is_some()
    {
        "get-symbolic"
    } else if action
        .downcast_ref::<model::ContainerCopyToAction>()
        .is_some()
    {
        "put-symbolic"
    } else if action
    ...
    } else if action.downcast_ref::<model::ImageBuildAction>().is_some()
        || action.downcast_ref::<model::ImagePullAction>().is_some()
        || action.downcast_ref::<model::ImagePushAction>().is_some()
    {
        "image-x-generic-symbolic"
    } else if action.downcast_ref::<model::PodCreateAction>().is_some() {
        "pods-symbolic"
    } else if action.downcast_ref::<model::VolumeCreateAction>().is_some() {
...
fn action_description(action: &model::Action) -> String {
