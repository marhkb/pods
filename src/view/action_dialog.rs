...
mod imp {
    ...
    impl ObjectImpl for ActionDialog {
        ...
        fn constructed(&self) {
            ...
            let action_state_expr = action_expr.chain_property::<model::Action>("state");

            action_state_expr
                .chain_closure::<String>(closure!(|_: Self::Type, state: model::ActionState| {
                    match state {
                        model::ActionState::Cancelled => gettext("Cancelled"),
                        model::ActionState::Failed => gettext("Failed"),
                        model::ActionState::Finished => gettext("Finished"),
                        model::ActionState::Ongoing => gettext("Ongoing"),
                    }
                }))
    ...
    impl ActionDialog {
        fn set_action(&self, action: Option<&model::Action>) {
            let obj = &*self.obj();
...
impl From<model::Action> for ActionDialog {
    fn from(value: model::Action) -> Self {
        if let Some(action) = value.downcast_ref::<model::ContainerCommitAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ContainerCopyFromAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ContainerCopyToAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ContainerCreateAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ContainersPruneAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ImageBuildAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ImagePullAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ImagePushAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::ImagesPruneAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::PodCreateAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::PodsPruneAction>() {
            Self::from(action)
        } else if let Some(action) = value.downcast_ref::<model::VolumeCreateAction>() {
            Self::from(action)
...
impl ActionDialog {
    ...
    fn update_view(&self, action: &model::Action) {
        let imp = self.imp();

        match action.state() {
            model::ActionState::Cancelled => {
        ...
        self.action_set_enabled(ACTION_CANCEL, action.state() == model::ActionState::Ongoing);
        self.action_set_enabled(
            ACTION_VIEW_ARTIFACT,
            action.state() == model::ActionState::Finished
                && action
        ...
        self.action_set_enabled(
            ...
            matches!(
                ...
                model::ActionState::Cancelled
                    | model::ActionState::Failed
                    | model::ActionState::Finished
            ),
        );
    }

    fn update_time_label(&self, action: &model::Action) -> glib::ControlFlow {
    ...
    fn edit(&self) {
        ...
        let dialog: adw::Dialog = if let Some(action) =
            action.downcast_ref::<model::ContainerCommitAction>()
        {
            match action.container() {
                Some(container) => {
                    view::ContainerCommitOptsDialog::new(&container, Some(action.opts())).upcast()
                }

                None => alert_dialog(
                    &gettext("Container Removed"),
                    &gettext("Container cannot be committed because it has been removed."),
                ),
            }
        } else if let Some(action) = action.downcast_ref::<model::ContainerCopyFromAction>() {
            match action.container() {
                Some(container) => view::ContainerCopyFromOptsDialog::new(
                    &container,
                ...
                None => alert_dialog(
                    &gettext("Container Removed"),
                    &gettext(
                        "Files cannot be copied anymore because the container has been removed.",
                    ),
                ),
            }
        } else if let Some(action) = action.downcast_ref::<model::ContainerCopyToAction>() {
            match action.container() {
                Some(container) => view::ContainerCopyToOptsDialog::new(
                    &container,
                    action.directory(),
                    &action.host_path(),
                    &action.container_path(),
            ...
            view::PodCreateOptsDialog::new(&client, Some(action.opts())).upcast()
            ...
            view::ImageBuildOptsDialog::new(&client, Some(action.opts())).upcast()
            ...
            match action.repo_tag() {
                Some(repo_tag) => {
                    view::RepoTagPushOptsDialog::new(&repo_tag, Some(action.opts())).upcast()
                }
                None => alert_dialog(
                    &gettext("Tag Removed"),
                    &gettext("Cannot push removed tag."),
                ),
            }
            ...
            view::VolumeCreateOptsDialog::new(&client, Some(action.opts())).upcast()
...
fn alert_dialog(heading: &str, body: &str) -> adw::Dialog {
