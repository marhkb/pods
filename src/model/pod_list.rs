...
impl PodList {
                    ...
                    Ok(inspect_response) => {
                        ...
                        match entry {
                            Entry::Vacant(entry) => {
                                ...
                                pod.connect_notify_local(
                                    Some("name"),
                                    clone!(@weak obj => move |pod, _| {
                                        obj.pod_name_changed(pod);
                                    })
                                );
    ...
    fn pod_name_changed(&self, pod: &model::Pod) {
        self.emit_by_name::<()>("pod-name-changed", &[pod]);
    }

    fn pod_removed(&self, model: &model::Pod) {
        self.emit_by_name::<()>("pod-removed", &[&model]);
    }
    ...
    pub(crate) fn connect_pod_name_changed<F: Fn(&Self, &model::Pod) + 'static>(
        &self,
        f: F,
    ) -> glib::SignalHandlerId {
        self.connect_local("pod-name-changed", true, move |values| {
            let obj = values[0].get::<Self>().unwrap();
            let pod = values[1].get::<model::Pod>().unwrap();
            f(&obj, &pod);
    ...
    pub(crate) fn connect_pod_removed<F: Fn(&Self, &model::Pod) + 'static>(
        &self,
        f: F,
    ) -> glib::SignalHandlerId {
        self.connect_local("pod-removed", true, move |values| {
            let obj = values[0].get::<Self>().unwrap();
            let pod = values[1].get::<model::Pod>().unwrap();
            f(&obj, &pod);
